...
# groups.
numPossSwePts <- length(sweOut$obs_mm)*(length(modTags)+1)
numPossSdPts <- length(sdOut$obs_mm)*(length(modTags)+1)

# Create output dataframes
sweOutPts <- data.frame(matrix(NA,ncol=7,nrow=numPossSwePts))
sdOutPts <- data.frame(matrix(NA,ncol=7,nrow=numPossSdPts))

names(sweOutPts) <- c('uniqueId','lat','lon','region','POSIXct','value_mm','tag')
names(sdOutPts) <- c('uniqueId','lat','lon','region','POSIXct','value_mm','tag')
...
for (day in 1:nSteps){
   ...
   for (station in 1:length(uniqueTmp)){
      ...
      for (tag in 1:length(modTags)){
         ...
         sweOutPts$region[count] <- metaOut$region[station]
         count <- count + 1
      } 
   }
...
for (day in 1:nSteps){
   ...
   for (station in 1:length(uniqueTmp)){
      ...
      for (tag in 1:length(modTags)){
         ...
         sdOutPts$region[count] <- metaOut$region[station]
         count <- count + 1
      }
   }
