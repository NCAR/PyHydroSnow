...
def checkArgs(parser):
    ...
    if parser.endADate:
        ...
        if not parser.begADate:
            ...
            raise
    if not parser.jobName:
        print "ERROR: Please specify job name for this job."
        raise
    elif len(parser.jobName) == 0:
    ...
    if parser.bsnMskFile:
        if len(parser.bsnMskFile) == 0:
            ...
            raise
    if parser.snRead:
        if int(parser.snRead) == 3 and not parser.bsnMskFile:
            print "ERROR: Mask file necessary for aggregated reads."
            raise
        if int(parser.snRead) == 4 and not parser.bsnMskFile:
            print "ERROR: Mask file necessary for aggregated reads."
            raise
        if int(parser.snRead) == 5 and not parser.bsnMskFile:
            print "ERROR: Mask file necessary for aggregated reads."
            raise
        if int(parser.snRead) == 6 and not parser.bsnMskFile:
            print "ERROR: Mask file necessary for aggregated reads."
            raise
...
def mkJobDir(args,dbIn):
    ...
    # If job directory has already been created, proceed.
